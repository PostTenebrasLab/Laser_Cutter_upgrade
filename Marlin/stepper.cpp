...
#ifdef LASER
static char * raster_buff_pos;
// sample buffer
static char raster_buffer[]{
   ,
...
//      #endif
  ...
  if (current_block != NULL) {
    ...
    for(int8_t i=0; i < step_loops; i++) { // Take multiple steps per interrupt (For high speed moves)
      ...
      #ifdef LASER
          // TODO intensity could be 0; LASER_MIN_INTENSITY..LASER_MAX_INTENSITY
      if (laserMode == RASTER) {
//        (raster_buff_pos < 100) ? WRITE_DAC0(100) : 
        WRITE_DAC0(*(raster_buff_pos++));
        counter_e -= current_block->step_event_count;
      }
      #else
    ...
    if (step_events_completed <= (unsigned long int)current_block->accelerate_until) {
      ...
      #ifdef ADVANCE
        f or(int8_t i=0; i < step_loops; i++) {
...
//  dacc_set_power_save(DACC, 0, 0);      // take less time - no need to wake up
  ...
  dacc_write_conversion_data(DACC, 0x00);   // set laserIntensity to 0
  #if defined(E0_ENABLE_PIN) && (E0_ENABLE_PIN > -1)
    SET_OUTPUT(E0_ENABLE_PIN);
  #endif
